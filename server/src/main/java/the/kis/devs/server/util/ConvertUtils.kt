...
object ConvertUtils {
    ...
    fun isFile(
        byteArray : ByteArray
    ) : Boolean = byteArray.toString(Charsets.UTF_8).contains(fileRegex)
    ...
    fun isText(
        byteArray : ByteArray
    ) : Boolean = byteArray.toString(Charsets.UTF_8).contains(textRegex)
    ...
    fun textToByteArray(
        text : String
    ) : ByteArray = textRegex.toString().toByteArray() + text.toByteArray()
}