...
public class GameSenseHoleUtil {
    ...
    public static HoleInfo isHole(BlockPos centreBlock, boolean onlyOneWide, boolean ignoreDown) {
        ...
        HashMap<GameSenseHoleUtil.BlockOffset, GameSenseHoleUtil.BlockSafety> unsafeSides = GameSenseHoleUtil.getUnsafeSides(centreBlock);

        if (unsafeSides.containsKey(GameSenseHoleUtil.BlockOffset.DOWN)) {
            if (unsafeSides.remove(GameSenseHoleUtil.BlockOffset.DOWN, GameSenseHoleUtil.BlockSafety.BREAKABLE)) {
                if (!ignoreDown) {
        ...
        unsafeSides.entrySet().removeIf(entry -> entry.getValue() == GameSenseHoleUtil.BlockSafety.RESISTANT);
    ...
    private static HoleInfo isDoubleHole(HoleInfo info, BlockPos centreBlock, BlockOffset weakSide) {
        ...
        HashMap<GameSenseHoleUtil.BlockOffset, GameSenseHoleUtil.BlockSafety> unsafeSides = GameSenseHoleUtil.getUnsafeSides(unsafePos);
        ...
        if (unsafeSides.containsKey(GameSenseHoleUtil.BlockOffset.DOWN)) {
            ...
            unsafeSides.remove(GameSenseHoleUtil.BlockOffset.DOWN);
        }
