...
public class CrystalUtils {
    ...
    private static boolean canPlaceCrystal(final Object pos) {
        BlockPos _pos = (BlockPos) pos;

        final Block block = mc.world.getBlockState(_pos).getBlock();
        if (block == Blocks.OBSIDIAN || block == Blocks.BEDROCK) {
            final Block floor = mc.world.getBlockState(_pos.add(0, 1, 0)).getBlock();
            final Block ceil = mc.world.getBlockState(_pos.add(0, 2, 0)).getBlock();
            if (floor == Blocks.AIR && ceil == Blocks.AIR && mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(_pos.add(0, 1, 0))).isEmpty()) {
                return true;
            }
        }
        return false;
...
//        positions.addAll((Collection) BlockInteractionHelper.getSphere(GetPlayerPosFloored(p_Player), p_Range, (int)p_Range, false, true, 0).stream().filter((Predicate<? super Object>)CrystalUtils::canPlaceCrystal).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
    ...
    private static float getDamageMultiplied(final World p_World, final float damage) {
        final int diff = p_World.getDifficulty().getDifficultyId();
