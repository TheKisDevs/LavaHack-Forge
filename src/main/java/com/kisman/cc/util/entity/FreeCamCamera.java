...
public class FreeCamCamera extends EntityPlayerSP {
    ...
    public FreeCamCamera(World world) {
        super(mc, world, mc.getConnection(), mc.player.getStatFileWriter(), mc.player.getRecipeBook());
    }
    ...
    public void onLivingUpdate() {
        ...
        if (follow) {
            if (Math.abs(this.motionX) <= 1e-8f) this.posX += (mc.player.posX - mc.player.prevPosX);
            if (Math.abs(this.motionY) <= 1e-8f) this.motionY += (mc.player.posY - mc.player.prevPosY);
            if (Math.abs(this.motionZ) <= 1e-8f) this.motionZ += (mc.player.posZ - mc.player.prevPosZ);
        }
