...
import com.kisman.cc.features.module.player.rotatemodifier.YawLock
import com.kisman.cc.features.module.player.rotatemodifier.YawStep
...
class RotateModifier : Module() {
    ...
    private val bowAim = register(Setting("Bow Aim", this, false))
    ...
    private fun doBowAim() {
        if(bowAim.valBoolean) {
            if(nearest() != null && mc.player.heldItemMainhand.item is ItemBow && mc.player.isHandActive && mc.player.itemInUseCount >= 3) {
                RotationSystem.handleRotate(nearest()!!)
            }
        }
    }
