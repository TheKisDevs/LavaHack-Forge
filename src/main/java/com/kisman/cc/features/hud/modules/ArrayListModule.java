...
public class ArrayListModule extends ShaderableHudModule {
    ...
    public void draw() {
        ...
        if(modules.getValBoolean()) for(Module mod : Kisman.instance.moduleManager.modules) if(mod != null && (mod.isToggled() || (animation.getValBoolean() && mod.getXCoeff() > 0)) && mod.visible) elements.add(new ArrayListElement(mod, (mod.getName() + (mod.getDisplayInfo().isEmpty() || !showDisplayInfo.getValBoolean() ? "" : " " + TextFormatting.GRAY + mod.getDisplayInfo())), (mod.displayName + (mod.getDisplayInfo().equalsIgnoreCase("") ? "" : " " + mod.getDisplayInfo())), ElementTypes.Module));
        if(hudModules.getValBoolean()) for(HudModule mod : Kisman.instance.hudModuleManager.modules) if(mod != null && (mod.isToggled() || (animation.getValBoolean() && mod.getXCoeff() > 0)) && mod.visible) elements.add(new ArrayListElement(mod, (mod.getName() + (mod.getDisplayInfo().isEmpty() || !showDisplayInfo.getValBoolean() ? "" : " " + TextFormatting.GRAY + mod.getDisplayInfo())), (mod.displayName + (mod.getDisplayInfo().equalsIgnoreCase("") ? "" : " " + mod.getDisplayInfo())), ElementTypes.HudModule));
