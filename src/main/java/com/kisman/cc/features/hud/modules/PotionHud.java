...
public class PotionHud extends HudModule {
    ...
    private final HashMap<String, Pair<PotionEffect, Integer>> potions = new HashMap<>();
        ...
        if(event.getPacket() instanceof SPacketEntityEffect) {
            ...
            if(mc.player.entityId == packet.getEntityId()) {
                ...
                if(potions.containsKey(effect.getEffectName())) {
                    Pair<PotionEffect, Integer> pair = potions.get(effect.getEffectName());
                    pair.getKey().combine(effect);
                    potions.put(effect.getEffectName(), pair);
                } else potions.put(effect.getEffectName(), new Pair<>(effect, effect.getDuration()));
            }
    ...
    public void onRender(RenderGameOverlayEvent.Text event){
        ...
        for(PotionEffect effect : effects) {
            ...
            if(sliders.getValBoolean() && potions.containsKey(effect.getEffectName())) {
                double sliderWidth = (getW() + 10) * ((double) potions.get(effect.getEffectName()).getKey().getDuration() / (double) potions.get(effect.getEffectName()).getValue());
