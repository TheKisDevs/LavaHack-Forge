...
public class SchematicStructure extends SchematicFormat {
    ...
    public ISchematic readFromNBT(final NBTTagCompound tagCompound) {
        ...
        for (Template.BlockInfo block : ((ITemplate) template).blocks()) {
    ...
    public boolean writeToNBT(final NBTTagCompound tagCompound, final ISchematic schematic) {
        ...
        ((ITemplate) template).size(new BlockPos(schematic.getWidth(), schematic.getHeight(), schematic.getLength()));
        ...
        for (BlockPos pos : BlockPos.getAllInBox(BlockPos.ORIGIN, template.getSize().add(-1, -1, -1))) {
            ...
            ((ITemplate) template).blocks().add(new Template.BlockInfo(pos, schematic.getBlockState(pos), compound));
        }
        ...
        for (Entity entity : schematic.getEntities()) {
            try {
                ...
                ((ITemplate) template).entities().add(new Template.EntityInfo(vec3d, blockpos, nbttagcompound));
