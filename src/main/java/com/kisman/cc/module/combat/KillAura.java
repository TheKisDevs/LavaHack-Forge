    ...
    public void update() {
        ...
        if(mode.getValString().equalsIgnoreCase("Multi")) {
                ...
                if (mc.world.loadedEntityList.get(i) != null && ((mc.world.loadedEntityList.get(i) instanceof EntityPlayer && player) || (mc.world.loadedEntityList.get(i) instanceof EntityMob && monster) || (mc.world.loadedEntityList.get(i) instanceof EntityAnimal && passive))) {
                    ...
                    if (mc.player.getDistance(mc.world.loadedEntityList.get(i)) <= 4.15 && mc.world.loadedEntityList.get(i).ticksExisted % 20 == 0 && mc.world.loadedEntityList.get(i) != mc.player) {
                        ...
                        if(entity instanceof EntityPlayer && shieldBreaker.getValBoolean()) {
                            ...
                            if(entity1.getHeldItemMainhand().getItem() instanceof ItemShield || entity1.getHeldItemOffhand().getItem() instanceof ItemShield) if(entity1.isHandActive()) isShieldActive = true;
                        }
                        ...
                        } else if(mc.player.inventory.currentItem != weaponSlot) return;
           ...
           if(mc.player.getDistance(target) <= 4.15 && target.ticksExisted % 20 == 0) {
               ...
               if(shieldBreaker.getValBoolean()) if (target.getHeldItemMainhand().getItem() instanceof ItemShield || target.getHeldItemOffhand().getItem() instanceof ItemShield) if (target.isHandActive()) isShiendActive = true;
               ...
               if (this.hitsound && isHit) mc.getSoundHandler().playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.BLOCK_STONE_BREAK, 1));
               if(switchMode.getValString().equalsIgnoreCase("Silent") && oldSlot != -1) InventoryUtil.switchToSlot(oldSlot, true);
           }
        }
    }
