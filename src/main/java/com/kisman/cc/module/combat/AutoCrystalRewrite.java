...
public class AutoCrystalRewrite extends Module {
    ...
    private final Setting placeRange = new Setting("PlaceRange", this, 0, 4.5f, 6, false);
    private final Setting breakRange = new Setting("BreakRange", this, 0, 4.5f, 6, false);
    private final Setting wallPlaceRange = new Setting("WallPlaceRange", this, 0, 4, 6, false);
    private final Setting wallBreakRange = new Setting("WallBreakRange", this, 0, 4, 6, false);
    private final Setting targetRange = new Setting("TargetRange", this, 0, 30, 50, false);
    private final Setting rayTrace = new Setting("RayTrace", this, RayTrace.None);
    private final Setting swing = new Setting("Swing", this, SwingMode.Mainhand);
    ...
    private final Setting placeDelay = new Setting("PlaceDelay", this, 20, 0, 200, true);
    private final Setting breakDelay = new Setting("BreakDelay", this, 40, 0, 200, true);
    private final Setting clearDelay = new Setting("ClearDelay", this, 10, 1, 60, true);

    private final Setting placeLine = new Setting("PlaceLine", this, "Place");
    private final Setting place = new Setting("Place", this, true);
    private final Setting newVersionPlace = new Setting("1.13", this, false);
    private final Setting multiPlace = new Setting("MultiPlace", this, false);
    private final Setting terrainIgnore = new Setting("Terrain Ignore", this, true);
    ...
    private final Setting breakMode = new Setting("BreakMode", this, BreakMode.Always);
    private final Setting breakCalc = new Setting("BreakCalc", this, true);

    private final Setting rotateLine = new Setting("RotateLine", this, "Rotate");
    private final Setting rotate = new Setting("Rotate", this, Rotations.None);
    private final Setting clientSide = new Setting("ClientSize", this, false);
    private final Setting randomize = new Setting("Ramdomize", this, true);
    private final Setting yawStep = new Setting("YawStep", this, YawStep.None);
    private final Setting steps = new Setting("Steps", this, 0.3f, 0, 1, false);

    private final Setting damage = new Setting("DamageLine", this, "Damage");
    private final Setting minDMG = new Setting("MinDMG", this, 6, 0.1f, 36, false);
    private final Setting maxSelfDMG = new Setting("MaxSelfDMG", this, 12, 0.1f, 36, false);
    private final Setting facePlaceHP = new Setting("FacePlaceDMG", this, 0, 0, 37, true);
    private final Setting armor = new Setting("Armor Scale", this, 12, 0, 100, true);

    private final Setting switchLine = new Setting("SwitchLine", this, "Switch");
    private final Setting switchMode = new Setting("Switch", this, SwapMode.None);
    private final Setting antiWeaknessSwitchMode = new Setting("AntiWeaknessSwitch", this, SwapMode.None);

    private final Setting pauseLine = new Setting("PauseLine", this, "Pause");
    private final Setting pauseIfEating = new Setting("Pause If Eating", this, false);
    private final Setting pauseIfHealth = new Setting("Pause If Heaith", this, false);
    private final Setting requirestHealth = new Setting("Requirest Health", this, 5, 0, 37, true);
...
//    @SubscribeEvent
        ...
        if(event.getPacket() instanceof SPacketSpawnObject && instant.getValBoolean()) {
            ...
            if (packet.getType() == 51) {
                ...
                for (BlockPos pos : placedCrystals) {
                    ...
                    if (!canSee && (rayTrace.getValString().equals(RayTrace.Full.name()) || rayTrace.getValString().equals(RayTrace.Hit.name()))) break;

                    if (mc.player.getDistance(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5) >= (canSee ? breakRange.getValDouble() : wallBreakRange.getValDouble()))
                        break;

                    if (breakMode.getValString().equals(BreakMode.Own.name()) && Math.sqrt(getDistance(pos.getX(), pos.getY(), pos.getZ(), packet.getX(), packet.getY(), packet.getZ())) > 1.5)
                        continue;

                    if (breakMode.getValString().equals(BreakMode.Smart.name()) && EntityUtil.getHealth(mc.player) - CrystalUtils.calculateDamage(mc.world, packet.getX(), packet.getY(), packet.getZ(), mc.player, terrainIgnore.getValBoolean()) < 0)
                        break;
                    ...
                    if (rotate.getValString().equals(Rotations.Full.name())) {
    ...
    private final Listener<EventPlayerMotionUpdate> listener1 = new Listener<>(event -> {
        if(rotation != null && !rotate.getValString().equals(Rotations.None.name()) ) {
            if(!yawStep.getValString().equals(YawStep.None.name()) && steps.getValDouble() < 1) {
    ...
    private boolean needPause() {
        ...
        return pauseIfHealth.getValBoolean() && mc.player.getHealth() <= requirestHealth.getValInt();
    }
    ...
    private void doPlace() {
        ...
        if(!rotate.getValString().equals("None")) {
        ...
        if(oldSlot != -1 && switchMode.getValString().equals("Silent")) {
    ...
    private void doBreak() {
        ...
        if(mc.player.isPotionActive(MobEffects.WEAKNESS)) {
            ...
            switch (antiWeaknessSwitchMode.getValString()) {
                case "None": {
        ...
        if(rotate.getValString().equals("Full")) {
            rotation = RotationManager.calcRotation(crystal);
        }

        breakCrystal(crystal);

        if(mc.player.isPotionActive(MobEffects.WEAKNESS) && oldSlot != -1 && switchMode.getValString().equals(SwapMode.Silent.name())) InventoryUtil.switchToSlot(oldSlot, true);
    }
    ...
    public void swing() {
        if(swing.getValString().equals(SwingMode.None.name())) return;

        if(packetSwing.getValBoolean()) {
            mc.player.connection.sendPacket(new CPacketAnimation(swing.getValString().equals(SwingMode.Mainhand.name()) ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND));
        } else {
            mc.player.swingArm(swing.getValString().equals(SwingMode.Mainhand.name()) ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND);
        }
    }

    private boolean isCrystalValid(EntityEnderCrystal e) {
        boolean canSee = mc.player.canEntityBeSeen(e);
        if (!canSee && (rayTrace.getValString().equals(RayTrace.Full.name()) || rayTrace.getValString().equals(RayTrace.Hit.name()))) return false;

        if (breakMode.getValString().equals(BreakMode.Own.name())) {
            for (BlockPos blockPos : placedCrystals) {
                if (e.getDistance(blockPos.getX(), blockPos.getY(), blockPos.getZ()) <= 3) continue;
                return false;
            }
        } else if (breakMode.getValString().equals(BreakMode.Smart.name()) && EntityUtil.getHealth(mc.player) - CrystalUtils.calculateDamage(mc.world, e.posX, e.posY, e.posZ, mc.player, terrainIgnore.getValBoolean()) < 0) {
            return false;
        }
    ...
    private boolean isPosValid(BlockPos pos) {
        boolean canSee = EntityUtil.canSee(pos);
        if (!canSee && (rayTrace.getValString().equals(RayTrace.Full.name()) || rayTrace.getValString().equals(RayTrace.Place.name()))) return false;
